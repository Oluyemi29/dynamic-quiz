...
import DeleteEndQuiz from "./delete/DeleteEndQuiz";
...
const AdminEndedQuiz = ({ department, quizOn }: DepartProps) => {
  ...
  const OnQuiz = AllQuiz.filter((AQuizes) => {
    ...
    // const startDate = new Date(AQuizes.startDate);
  ...
  return (
    <div>
      ...
      <h1 className="mt-5 font-semibold text-faidBlue">
        {adminDepartment?.department} Students End Quiz
      </h1>
      <Table className="mt-5 ">
        <TableCaption className="font-semibold text-faidBlue">
          {OnQuiz.length > 0 ? "List Of Ended Quiz" : "No Ended Quiz currently"}
        </TableCaption>
        <TableHeader>
